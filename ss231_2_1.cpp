    ...
    for (int T = 1; T <= K; T++) {
        ...
        for (int i = 1; i <= M; i++) {
            if (!isRunner[i])
                continue;
            int dir = 0;
           // cout << i << " 참가자 위치 : " << runner[i][0] << ", " << runner[i][1] << endl;
            ...
            for (int j = 1; j <= 4; j++) {
                ...
                if (distance < min_distance) {
                    //cout << "기존 거리 : " << min_distance << " " << i << " 거리 : " << distance << endl;
                    min_distance = distance;
                    dir = j;
                }
            }   
            if (dir != 0) {
                cout << i << " 참가자 이동 -> " << dir << endl;
        ...
        cout << "정사각형 길이와 참가자 인덱스" << endl;
        cout << square_len << " , " << square_idx << endl;
        // 정사각형 정확한 위치 찾기
        bool found = false;
        for (int i = 1; i <= N - square_len + 1; i++) {
            for (int j = 1; j <= N - square_len + 1; j++) {
                ...
                if (found) {
                    cout << "square 찾음 : " << square.first << ", " << square.second << " len : " << square_len << endl;
                    break;
                }
            }
            if (found) 
