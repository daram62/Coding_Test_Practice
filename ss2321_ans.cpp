...
int main() {
    ...
    for(int t = 1; t <= m; t++) {
        ...
        for(int i = 1; i <= p; i++) {
            ...
            pair<int, pair<int, int> > currentBest = make_pair((closestX - rudolf.first) * (closestX - rudolf.first) + (closestY - rudolf.second) * (closestY - rudolf.second), make_pair(-closestX, -closestY));
            pair<int, pair<int, int> > currentValue = make_pair((pos[i].first - rudolf.first) * (pos[i].first - rudolf.first) + (pos[i].second - rudolf.second) * (pos[i].second - rudolf.second), make_pair(-pos[i].first, -pos[i].second));
            ...
            if(rudolf.first == closestX && rudolf.second == closestY) {
                ...
                while(!(lastX == firstX and lastY == firstY)) {
                    ...
                    if(!is_inrange(lastX, lastY)) {
                        ...
                        pos[idx] = pair<int, int>(lastX, lastY);
                    }
                ...
                points[closestIdx] += c;
                pos[closestIdx] = pair<int, int>(firstX, firstY);
        ...
        cout << t << "턴이 끝난 후 게임판" << endl;
        for(int i = 1; i <= p; i++) 
        {
            if(is_live[i]) points[i]++;
            cout << i << "번째 산타의 점수 : " << points[i] << endl;
        }    
        Print();
