...
vector <int> adj_knight; // 움직일 기사 저장 배열 
...
int main() {
    ...
    {   
        ...
        cout << k_idx << " 기사가 " << dir << " 방향 이동" << endl;
        // 이동할 기사가 탈락했다면 그냥 무시하고 다음 명령 진행
        if (!isKnight[k_idx]) 
            continue;
        if(CanMove(k_idx, dir)) {
            cout << k_idx << " 이동 확정" << endl;
        ...
        for (int j = ny; j < ny+k_shield[idx][1]; j++) {
            if (k_board[i][j] != 0 && k_board[i][j] != idx) {
                ...
                if(!CanMove(k_board[i][j], dir)) {
                    cout << idx << " 기사 밀려나는 과정에서 이동 불가" << endl;
                    return false;
                }
            }
        }
